...
| IEEE 1788 | GAOL | Algorithm | Tightness | Tests |
|---|---|---|---|---|
| `pown(x, n)` | `pow(x, n)` for an int n, and `pow(x, y)` for a degenerate integer y | Binary exponentiation of the bounds, each product rounded outward; x<sup>−n</sup> is 1/x<sup>n</sup>. n − 1 rounded products move a bound at most (n − 1)·2<sup>−52</sup> relatively | n = 0, 1, 2: tightest. n ≥ 3: valid, within about 2(n − 1) doubles; accurate for n = 3. Negative n: valid, one rounded division more | n: within n doubles; −n: within 4 doubles for n ≤ 3 |
| `pow(x, y)` | `pow(x, y)`, `pow(x, d)` for a non-integer y | exp(y·log(x ∩ [0, +∞])): `log` and `exp` within one double each, and a rounded product; 0<sup>y</sup> = 0 for y > 0, ∅ for y ≤ 0 | valid: bounds within about (3 + 2\|y log x\|)·2<sup>−52</sup> relatively; exact where x = [1] | within 2<sup>−49</sup>·max(1, \|y log x\|) relatively; 4 doubles on the special cases |
...
| IEEE 1788 | GAOL | Algorithm | Tightness | Tests |
|---|---|---|---|---|
| `cos` | `cos(x)` | The bounds divided by an enclosure of π, rounded so as to never miss an extremum, tell whether x reaches −1 or 1; otherwise mathlib's cos at the bounds, correctly rounded to nearest, moved one double outward, within [−1, 1] | accurate for \|x\| ≤ 2<sup>25</sup>: within one double. Beyond, a bound within about \|x\|·2<sup>−51</sup> of a multiple of π may be taken as −1 or 1, at most about \|x\|<sup>2</sup>·2<sup>−103</sup> away from the exact bound: valid | within 1 double for \|x\| ≤ 2<sup>25</sup>; within \|x\|<sup>2</sup>·2<sup>−103</sup> + 2<sup>−51</sup> beyond |
| `sin` | `sin(x)` | As cos, the bounds divided by an enclosure of π, minus 1/2, telling the pieces where sin is monotonic; mathlib's sin at the bounds, moved one double outward, within [−1, 1] | accurate for \|x\| ≤ 2<sup>25</sup>: within one double. Beyond, a bound within about \|x\|·2<sup>−51</sup> of an odd multiple of π/2 may be taken as −1 or 1, as for cos: valid | within 1 double for \|x\| ≤ 2<sup>25</sup>; within \|x\|<sup>2</sup>·2<sup>−103</sup> + 2<sup>−51</sup> beyond |
| `tan` | `tan(x)` | [−∞, +∞] when x is wider than π, beyond [−2<sup>52</sup>, 2<sup>52</sup>], or not surely within one branch, x + [π/2] being divided by [π]; otherwise mathlib's tan at the bounds, moved one double outward | accurate within a branch, away from its ends by more than the width of [π] times the branch number: within one double. [−∞, +∞] otherwise: valid | within 2<sup>−49</sup>·max(1, \|x\|)·(1 + tan<sup>2</sup>) |
| `asin`, `acos` | `asin(x)`, `acos(x)` | mathlib's asin and acos at the bounds of x ∩ [−1, 1], moved one double outward | accurate: within one double | within 1 double |
| `atan` | `atan(x)` | mathlib's atan at the bounds, moved one double outward | accurate: within one double | within 1 double |
| `atan2` | `atan2(y, x)` | Not implemented: throws `unavailable_feature_error` | | |
| `sinh`, `tanh`, `asinh` | `sinh(x)`, `tanh(x)`, `asinh(x)` | The libm of the system at the bounds (mathlib has none), rounded to nearest, moved three doubles outward; tanh within [−1, 1] | valid as long as the libm is within two doubles of the exact value; bounds within 3 + e doubles, e being the error of the libm in doubles | within 4 doubles |
| `cosh` | `cosh(x)` | As sinh, at the bound of largest magnitude; 1 when x contains 0 | valid as above; 1 exact | within 3 doubles |
| `acosh`, `atanh` | `acosh(x)`, `atanh(x)` | As sinh, on x ∩ [1, +∞], respectively x ∩ [−1, 1] | valid as above | within 4 and 3 doubles |
...
| IEEE 1788 | GAOL | Algorithm | Tightness | Tests |
|---|---|---|---|---|
| `rootn(x, n)`, n > 0 | `nth_root(x, n)` | n = 1: x; n = 2: `sqrt`. Otherwise mathlib's pow at the bounds with an exponent rounded from 1/n, moved one double outward, on x (odd n, the root of a negative number being the opposite of the root of its magnitude) or x ∩ [0, +∞] (even n); the root of 0 is 0 | n = 1, 2: tightest. n a power of 2: accurate, within one double. Otherwise valid: the rounded exponent moves the root by about \|log x\|·2<sup>−53</sup>/n relatively, besides the double | within 8 doubles for x in [2<sup>−30</sup>, 2<sup>30</sup>], 233 for all doubles |
| others | — | Not provided | | |
