...
# GAOL, libieeep1788 and Solaris Studio, runs them, and writes the table of
# their results into doc/compare/special_cases.md (between its markers).
# Run setup.sh first.
set -euo pipefail
...
python3 "$CODE_DIR/cases.py" report "$OUT/gaol.txt" "$OUT/p1788.txt" "$OUT/sun.txt" "$REPORT"
