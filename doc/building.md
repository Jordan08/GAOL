...
| `GAOL_LTO` | `OFF` | Compile GAOL's own sources with interprocedural optimization (link-time optimization). The sources of CORE-MATH are always compiled apart, without it. The gain is on the basic operations and appears fully when the program using GAOL is compiled with it too (`-DCMAKE_INTERPROCEDURAL_OPTIMIZATION=ON` for the whole build): x + y took 1.48 ns rather than 3.02, x − y 1.35 rather than 3.11, `sqr(x)` 2.28 rather than 3.32, `sqrt(x)` 6.35 rather than 8.65 and `pow(x, 3)` 12.03 rather than 15.66 on an Intel i7-1185G7 with GCC 9, the elementary functions being unchanged. `OFF` by default: the objects of a static library compiled with it hold the intermediate representation of the compiler, so an installed GAOL would only link with the compiler that built it |
